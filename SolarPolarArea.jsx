...
export const SolarPolarArea = () => {
  useEffect(() => {
    const fetchDataAndDrawChart = async () => {
      try {
        ...
        const chartAttribute = document.createElement("canvas");
        chartCanvas.appendChild(chartAttribute);

        new Chart(chartAttribute, {
          type: "polarArea",
          data: {
            labels: xValues,
            datasets: [
          ...
          options: {
            ...
            plugins: {
              title: {
                display: true,
                text: `UNLV Solar Energy Production ${currentDate}`,
              },
  ...
  return (
    <div className="Solar-Container">
      <div className="Solar-Canvas" id="SolarChart" role="img"></div>
    </div>
  );
};
